...
import { getFirestore, doc, collection, addDoc, getDocs, deleteDoc, updateDoc } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
...
async function setAlarm() {
    ...
    try {
        ...
        if (editingAlarmId) {
            ...
            await updateDoc(alarmDocRef, { time, sound, label });
            editingAlarmId = null;
...
function displayAlarms() {
    ...
    alarms.forEach(alarm => {
        ...
        div.innerHTML = `
            ...
            <div class="alarm-controls" style="margin-top:8px;">
                <button data-id="${alarm.id}" class="edit-alarm-btn" title="Edit">Edit</button>
                <button data-id="${alarm.id}" class="delete-alarm-btn" title="Delete">Delete</button>
            </div>
        `;
    ...
    document.querySelectorAll(".edit-alarm-btn").forEach(btn => {
        btn.onclick = function() {
            ...
            if (alarm) {
                ...
                ringtoneSelect.value = alarm.sound;
